...
class GoldFlowerService(GameService):
    ...
    def on_server_connect(self,server_name,service_ids,is_connect):
        #print "server %s,%s,%s" % (server_name,service_ids,is_connect)
        if not is_connect:
            players = self.table_manager.get_players_by_access_services(service_ids)
            for p in players:
                table = self.get_table(p.uid)
                if table == None:
                    continue
                table.lock.acquire()
                try:
                    table.remove_player(p.uid)
                finally:
                    table.lock.release()
                print "------> Player[%d] is leaving ,since access service quit " % p.uid
