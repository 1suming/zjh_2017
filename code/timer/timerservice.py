    ...
    def handle_one_timer(self,timer):
        if timer.event_id in TIMER_HANDLERS:
            ...
            try:
                ...
                logging.exception("��ͨ��ʱ�������쳣 event=%d,user_id=%d,object_id=%d",timer.event_id,timer.uid,timer.object_id)
                session.rollback()
            finally:
                if session != None:
                    session.close()
